...
load_dotenv()
S3_LOCATION = osenv.get('S3_LOCATION')
...
def main():
    ...
    try:
        ...
        if len(rows) > 30000:  # QUICK check for validity of data
            ...
            # current_table_location = spark.sql("desc formatted fan_search.geonames_cities1000") \
            #     .filter("col_name=='Location'") \
            #     .collect()[0].data_type
            # if path in current_table_location:
            ...
            sparkDF.write.mode("overwrite").parquet(path)
            #
